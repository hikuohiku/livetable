...
const LiveTable = ({ lives, channels }: LiveTableProps) => {
  ...
  const tableColumnCount = useSyncExternalStore(
    ...
    () => {
      return tableRef.current?.clientWidth &&
        Math.floor(tableRef.current?.clientWidth / 288);
    },
    () => undefined,
  );
  ...
  const livesUpcomingGroupedByStartAtHour = livesUpcoming?.reduce<
    Record<string, Video[]>
  >((arr, live) => {
  ...
  return livesStreamingNow || livesUpcomingGroupedByStartAtHour
    ? (
      <div
        ref={tableRef}
        className="flex flex-col w-full max-w-screen-xl ml-12 gap-2"
      >
        {livesStreamingNow && (
          <LiveGroup
            lives={livesStreamingNow}
            channels={channels}
            columns={tableColumnCount}
          />
        )}
        {livesUpcomingGroupedByStartAtHour &&
          Object.entries(livesUpcomingGroupedByStartAtHour).map(
            ([key, values]) => {
              return (
                <React.Fragment key={key}>
                  {values[0]?.startAt && (
                    <Splitter
                      time={new Date(values[0].startAt.setMinutes(0, 0, 0))}
                    />
                  )}
                  <LiveGroup
                    lives={values}
...
const LiveGroup = ({ lives, channels, columns }: LiveGroupProps) => {
  ...
  const livesSortedByStartAt = lives &&
    lives.flat().toSorted((
      a,
      b,
    ) => (a.startAt
      ? (b.startAt ? a.startAt.getTime() - b.startAt.getTime() : -1)
      : 1)
    );
            ...
            <LiveCard
              ...
              channel={channels &&
                channels.find((channel) =>
                  channel?.channelId === live.channelId
                )}
              live={live}
            />
          );
