...
const LiveCard = ({ channel, live }: LiveCardProps) => {
  ...
  return (
    <div
      className={`glass ${backGroundColor} flex justify-center items-center gap-2 rounded-lg p-1`}
    >
      <div className="w-1/4">
        <ChannelThumbnail
...
const LiveThumbnail = (
  { liveThumbnail, liveUrl }: { liveThumbnail: string; liveUrl: string },
) => {
  const [hasError, setHasError] = useState(false);
